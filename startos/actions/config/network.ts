...
const networkSpec = InputSpec.of({
  network: Value.select({
    ...
    values: {
      mainnet:  'Mainnet',
      testnet:  'Testnet3 (legacy test network)',
  ...
  async ({ effects, input }) => {
    ...
    if (current === next) {
      return {
        ...
        message: `Flowee is already configured for ${next}.`,
        result: null,
      }
    }
    ...
    return {
      ...
      message: `Switched Flowee from ${current} to ${next}. Restarting automatically.`,
      result: null,
    }
  },
)
